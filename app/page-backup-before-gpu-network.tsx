...
export default function Home() {
        ...
        <div className="max-w-[1580px] mx-auto h-full px-6 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-9 text-sm font-black text-gray-100">
            {navItems.map((item, index) => (
              <a key={item} href="#" className="relative hover:text-lime-400">
                {item}
                {index === 0 && (
        ...
        <div className="relative overflow-hidden rounded-3xl border border-lime-400/25 bg-black/65 p-5 shadow-[0_0_55px_rgba(132,255,0,.12)]">
          ...
          <div className="relative grid lg:grid-cols-[.9fr_1.1fr] gap-5 items-center">
            ...
            <div className="relative h-[360px] rounded-3xl overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_42%,rgba(132,255,0,.40),transparent_58%)]" />
              <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(132,255,0,.045)_1px,transparent_1px),linear-gradient(rgba(132,255,0,.045)_1px,transparent_1px)] bg-[size:38px_38px]" />

              <div className="absolute top-5 left-5 rounded-xl border border-lime-400/35 bg-black/75 px-4 py-2 text-xs text-lime-400 font-black z-10">
                ● GPU NODE ACTIVE
              </div>
              ...
              <img
                src="/awf_neon_wolf.png"
                alt="Alpha Wolf"
                className="absolute left-1/2 top-[39%] w-[590px] max-w-none -translate-x-1/2 -translate-y-1/2 object-contain drop-shadow-[0_0_95px_rgba(132,255,0,.95)]"
              />

              <div className="absolute left-5 right-5 bottom-5 rounded-2xl border border-lime-400/35 bg-black/85 p-4">
          ...
          <div className="relative grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
            {stats.map(([num, label]) => (
              <div
                key={label}
                className="rounded-2xl border border-lime-400/20 bg-black/60 p-4"
              >
        ...
        <aside className="rounded-3xl border border-lime-400/30 bg-black/75 p-4 shadow-[0_0_45px_rgba(132,255,0,.13)]">
          ...
          <div className="grid grid-cols-4 gap-3 mt-2">
            {[
              ...
              <div
                key={label}
                className="rounded-xl border border-white/10 bg-zinc-900 p-2.5 text-center"
              >
          ...
          <div className="mt-2 grid grid-cols-3 gap-3">
            {payments.map(([label, symbol, color]) => (
              <button
                key={label}
              ...
              >
                <span
                  className={`w-7 h-7 rounded-full ${color} flex items-center justify-center text-xs font-black text-white`}
                >
                  {symbol}
                </span>
                {label}
          ...
          <div className="mt-3 grid grid-cols-3 gap-3 text-center">
            {[
              ...
              <div
                key={label}
                className="rounded-xl border border-lime-400/15 bg-lime-400/5 p-2.5"
              >
        ...
        <div className="min-h-[270px] h-full rounded-3xl border border-lime-400/20 bg-black/65 p-5 flex flex-col">
          ...
          <div className="mt-auto pt-5 grid grid-cols-[1fr_1fr_1fr_1.5fr] gap-3">
            {gpuStats.map(([value, label]) => (
              <div
                key={label}
                className="rounded-2xl border border-lime-400/20 bg-lime-400/5 p-4 text-center"
              >
        ...
        <div className="min-h-[270px] h-full rounded-3xl border border-lime-400/20 bg-black/65 p-5 flex flex-col">
          ...
          <div className="mt-auto pt-5 grid grid-cols-3 gap-3">
            {aiModules.map(([title, desc]) => (
              <div
                key={title}
                className="rounded-2xl border border-lime-400/20 bg-lime-400/5 p-3 text-center"
              >
  ...
  <div className="absolute inset-[18px] rounded-full bg-black border border-lime-400/40 flex items-center justify-center">
    <img
      src="/awf_neon_wolf.png"
      alt="AWF"
      className="h-[86px] w-[86px] object-contain drop-shadow-[0_0_28px_rgba(132,255,0,.9)]"
    />
  </div>
</div>
          </div>
