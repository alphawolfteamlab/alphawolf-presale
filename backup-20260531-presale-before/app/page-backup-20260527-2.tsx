...
export default function Home() {
  return (
    <main className="min-h-screen bg-[#020403] text-white overflow-hidden">
      ...
      <header className="relative z-10 border-b border-lime-400/20 bg-black/50 backdrop-blur-xl">
        <div className="max-w-[1500px] mx-auto px-5 py-5 flex items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full border border-lime-400 bg-lime-400/10 flex items-center justify-center shadow-[0_0_25px_rgba(163,230,53,.6)]">
              🐺
            </div>
            <div>
              <div className="text-2xl font-black text-lime-400 leading-none">ALPHA WOLF</div>
              <div className="text-xs tracking-[4px] text-lime-300">TOKEN (AWF)</div>
            </div>
          </div>
          ...
          <button className="px-6 py-3 rounded-xl bg-lime-400 text-black font-black shadow-[0_0_25px_rgba(163,230,53,.7)]">
            CONNECT WALLET
          </button>
        </div>
      </header>

      <section className="relative z-10 max-w-[1500px] mx-auto px-5 py-10 grid xl:grid-cols-[1.2fr_.55fr] gap-6">
        <div className="rounded-3xl border border-lime-400/20 bg-black/45 p-6 md:p-10 shadow-[0_0_60px_rgba(132,255,0,.12)]">
          <div className="grid lg:grid-cols-[1fr_.9fr] gap-8 items-center">
            <div>
              ...
              <p className="tracking-[8px] text-gray-300 mb-3">WELCOME TO</p>
              ...
              <p className="text-xl md:text-2xl mt-7 text-gray-200 leading-relaxed">
                Born to lead the pack.
                <br />
                <span className="text-lime-400 font-bold">GPU-powered</span> AI compute,
                rendering and utility ecosystem.
              </p>

              <div className="flex flex-wrap gap-4 mt-9">
                <button className="px-8 py-4 rounded-xl bg-lime-400 text-black font-black">
                  🐾 JOIN THE PACK
                </button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-10">
                {["REAL GPU POWER", "AI & RENDER", "REAL COMMUNITY", "UTILITY FIRST", "TRANSPARENT"].map((x) => (
                  <div key={x} className="rounded-2xl border border-lime-400/20 bg-lime-400/5 p-4">
                    ...
                    <div className="text-[11px] text-gray-400 mt-1">Built for long-term.</div>
                  </div>
                ))}
              </div>
            </div>

            <div className="relative min-h-[720px] rounded-3xl border border-lime-400/20 bg-[radial-gradient(circle,rgba(132,255,0,.25),transparent_55%)] flex items-center justify-center overflow-hidden">
  ...
  <img
    ...
    alt="Alpha Wolf"
    className="w-[420px] md:w-[520px] drop-shadow-[0_0_45px_rgba(132,255,0,.65)]"
  />
</div>

              <div className="absolute right-8 top-[55%] -translate-y-1/2 w-[320px] rounded-2xl border border-emerald-400/60 bg-black/85 p-5 backdrop-blur shadow-[0_0_35px_rgba(16,185,129,.25)]">
                <div className="text-lime-400 font-black">ALPHA WOLF AI</div>
                <div className="text-xs text-lime-300 mt-1">● ONLINE</div>
                <p className="text-sm mt-4 text-gray-200">
                  Welcome! I’m Alpha, your AI assistant. How can I help you today?
                </p>
                {["What is AWF?", "How does AI Factory work?", "Show me the GPU network"].map((q) => (
                  <button key={q} className="w-full mt-3 border border-lime-400/40 rounded-lg px-3 py-2 text-left text-xs text-lime-300">
                    {q}
                  </button>
                ))}
                <div className="mt-4 flex gap-2">
                  <input className="w-full bg-zinc-900 rounded-lg px-3 py-2 text-xs outline-none" placeholder="Ask me anything..." />
                  <button className="bg-lime-400 text-black rounded-lg px-3 font-black">➤</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mt-6">
            {stats.map(([num, label]) => (
              <div key={label} className="rounded-2xl border border-lime-400/20 bg-black/50 p-5">
        ...
        <aside className="rounded-3xl border border-lime-400/30 bg-black/70 p-7 shadow-[0_0_50px_rgba(132,255,0,.15)]">
          <div className="flex justify-between items-center">
            <h2 className="text-3xl font-black text-lime-400">AWF PRESALE</h2>
            <span className="border border-lime-400 rounded-lg px-3 py-1 text-lime-300 text-xs">STAGE 1</span>
          </div>

          <p className="mt-6 text-gray-300 text-sm">UNTIL PRICE INCREASE</p>
          <div className="grid grid-cols-4 gap-3 mt-3">
            {["15 DAYS", "08 HRS", "42 MIN", "18 SEC"].map((x) => (
              <div key={x} className="rounded-xl border border-white/10 bg-zinc-900 p-4 text-center font-black">
                {x}
              </div>
            ))}
          </div>
          ...
          <div className="mt-7 flex justify-between text-xl">
            ...
            <span>NEXT: <b className="text-lime-400">$0.15</b></span>
          </div>

          <p className="mt-7 text-gray-300 text-sm">PAY WITH</p>
          <div className="grid grid-cols-3 gap-3 mt-3">
            {["AVAX", "ETH", "USDT"].map((x) => (
              <button key={x} className="rounded-xl border border-white/10 bg-zinc-900 py-4 font-black">
                {x}
              </button>
            ))}
          </div>

          <button className="w-full mt-6 py-5 rounded-xl bg-lime-400 text-black font-black text-lg">
            CONNECT WALLET
          </button>
      ...
      <section className="relative z-10 max-w-[1500px] mx-auto px-5 pb-10 grid xl:grid-cols-[1.5fr_.9fr_.45fr] gap-6">
        ...
        <div className="rounded-3xl border border-lime-400/20 bg-black/60 p-6">
          <h2 className="text-2xl font-black text-lime-400">AI FACTORY <span className="text-xs border border-lime-400 rounded px-2">BETA</span></h2>
          ...
          <button className="border border-lime-400 text-lime-400 rounded-xl px-6 py-4 font-black">
            LAUNCH AI FACTORY
          </button>
        </div>
        ...
        <div className="rounded-3xl border border-lime-400/20 bg-black/60 p-6">
          ...
          {["🇹🇷 Türkiye 14", "🇺🇸 USA 8", "🇩🇪 Germany 5", "🇨🇦 Canada 3", "🇬🇧 UK 2"].map((x) => (
            <div key={x} className="mt-5 flex justify-between text-gray-300">{x}</div>
          ))}
        </div>
      </section>

      <footer className="relative z-10 border-t border-lime-400/20 py-8 text-center text-gray-400">
